...
function DuplicateWorkflowDialog({ workflowId }: { workflowId?: string }) {
  ...
  const {mutate,isPending} = useMutation({
    ...
    onError: () => {
      toast.error("Failer to duplicate workflow",{id:"duplicated-workflow"})
    },
  });
    ...
    <Dialog open={open} onOpenChange={(open)=>{
     ...
     setOpen(open);
      ...
      <DialogTrigger asChild>
        <Button variant={'ghost'} size={'icon'} className="ml-2 transition-opacity duration-200 opacity-0 group-hover/card:opacity-100"><CopyIcon  className="w-4 h-4 text-muted-foreground cursor-pointer"/></Button>
      </DialogTrigger>
              ...
              />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                        Provide a brief description of that your workflow does <br/>
